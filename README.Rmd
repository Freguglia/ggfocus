...
<!-- badges: start -->
[![Build Status](https://travis-ci.org/Freguglia/ggfocus.svg?branch=master)](https://travis-ci.org/Freguglia/ggfocus)
...
- `alpha_focus` and `alpha_other`: alpha for selcted and not selected levels, respectively.
 ...
 You can also use the usual `ggplot2` grammar `+` to add scales with the family of function `scale_*_focus()`. These two uses are equivalent, read the [vignette](vignettes/a-quick-guide-to-ggfocus.html) for more information.
...
Because `ggfocus()` retuns a modified `ggplot` object and only controls scales, other `ggplot` extensions and types of graphics can interact with it the same way, an example with `maps` is shown below.
